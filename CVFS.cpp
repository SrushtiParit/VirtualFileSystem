...
void man(char *name)
{
    ...
    else if(strcmp(name,"lseek") == 0)
    {
        ...
        printf("Usage: lseek File Name ChangeInOffset StartPoint\n");
    }
...
void ls_file()
{
    ...
    while(temp != NULL)
    {
        if(temp->FileType != 0)
        {
            printf("%s\t%d\t%d\t\t%d\n",temp->FileName,temp->FileName, temp->InodeNumber, temp->FileActualSize, temp->LinkCount);
        }
        temp = temp->next;
    }
    printf("-------------------------------------------------------\n");
...
int main()
{
    ...
    while(1)
    {
        ...
        if(count == 1)
        {
            ...
            {
                printf("Terminating the Srushti Virtual File System\n");
                break;
            }
            else
            {
        ...
        else if(count == 3)
        {
            ...
            {
                ret = CreateFile(command[1],atoi(command[2])); //atoi = ascii to integer(value ascii madhey ghetli pn tila integer banva) 
                if(ret >= 0)
